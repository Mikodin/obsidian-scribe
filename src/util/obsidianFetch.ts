 ...
 * avoid CORS issues with AI providers in the Obsidian renderer.
 * Typed structurally (not against a single SDK's Fetch type) so the OpenAI
 * SDK, LangChain's ChatOpenAI, the Anthropic SDK, and raw calls all accept it.
 *
 * Limitation: requestUrl buffers the entire response — no streaming. All
 * adapters must use non-streaming API calls.
 * @example
 * const client = new OpenAI({ fetch: obsidianFetch })
 * const model = new ChatOpenAI({ configuration: { fetch: obsidianFetch } })
 * const anthropic = new Anthropic({ fetch: obsidianFetch })
 */
export const obsidianFetch = async (
  ...
  // Groq (and some other OpenAI-compatible providers) reject JSON schemas that
  // include meta-fields like `$schema` or `title`.
  ...
  const obsidianParams: RequestUrlParam = {
    ...
    headers: headersObj,
    body: bodyBuffer,
    throw: false, // Don't throw on non-2xx — let the SDKs handle error responses
  };
  ...
  if (obsidianResponse.status >= 400) {
    // console.error, not debug: some providers (e.g. Gemini's OpenAI-compat
    // endpoint) return error bodies the SDKs can't parse, so this log is the
    // only place the real error message is visible.
    console.error('[obsidianFetch] error body:', obsidianResponse.text);
  }
